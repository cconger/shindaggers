...
func (s *Server) AdminIndex(w http.ResponseWriter, r *http.Request) {
	s.renderTemplate(w, "admin.html", nil)
}
...
func (s *Server) AdminDeleteKnife(w http.ResponseWriter, r *http.Request) {
	...
	knifeIDVal := r.FormValue("knife")
	if knifeIDVal == "" {
		http.Error(w, "knife must be specified", http.StatusBadRequest)
		return
	}

	knifeID, err := strconv.Atoi(knifeIDVal)
	if err != nil {
		http.Error(w, "knife id must be integer", http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		http.Error(w, fmt.Sprintf("error deleting %d, %s", knifeID, err.Error()), http.StatusInternalServerError)
		return
	}
